...
/**
* Front controller which handles provider auth callback methods and forwards to ProviderSignInController or ConnectController
* depending on user's logged in status. For this strategy to work, the application must ensure that calls to the ProviderSignInController
* (normally those for "/signin/[providerId]" urls are only accessible to non-logged in users.  This means that it is safe
* to assume in this controller that a callback for a non-logged in user be forwarded to the signin controller, while
* the callback for a logged-in user be forwarded to the connect controller.
* 
* This controller is used so that a single callback url ("/signinOrConnect") can be registered with providers which
* require exact match return urls and which only allow one such url (eg. SoundCloud).
*
...
public class ProviderSignInOrConnectController {
	...
	{
		String currentUserId = AuthenticatedUserIdHolder.getAuthenticatedUserId();
	...
	*/
	...
	public String oauth2Callback(@PathVariable String providerId, @RequestParam("code") String code, NativeWebRequest request) {
		return  "redirect:" + getRedirectPath() + "/" + providerId + "?code=" + code;
